...
class CursoAbiertoController extends Controller
{
    ...
    {
        $curso = CursoAbierto::with('catalogo')->findOrFail($id);
        $modulos = $curso->modulos()->paginate(15);
        return response()->json([
            'data' => $modulos->items(),
            'meta' => ['total' => $modulos->total()],
            'precio_inscripcion' => $curso->catalogo->precio_inscripcion ?? 0,
        ]);
    }
    ...
    public function asistenciaPDF($id): JsonResponse
    {
        ...
        $participantes = $curso->matriculas->map(function ($matricula) use ($allDates, $allClasesByDate) {
            ...
            $ciudadStr = $persona?->getAttribute('ciudad') ?? $externo?->getAttribute('ciudad') ?? '';
